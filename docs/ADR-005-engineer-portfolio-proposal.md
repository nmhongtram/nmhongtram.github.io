...
- Air Quality Forecasting has a public repository and no running demo. The owner confirms implementing OpenAQ ingestion and preprocessing, RNN/LSTM/GRU modeling, FastAPI inference, the Streamlit dashboard, and Docker packaging. The repository credits a UEH course group and names the owner as team leader; portfolio copy should preserve that group context while stating the owner's technical contribution. Describe it as a completed prototype unless current operation is independently established.
